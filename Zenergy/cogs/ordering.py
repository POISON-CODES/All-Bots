...
class Buttons(discord.ui.View):
       ...
       @discord.ui.button(label = 'Paid Tickets', style=discord.ButtonStyle.blurple, custom_id='Gray:GFX')
       async def GFX_button(self, button = discord.ui.Button, interaction = discord.Interaction):
              ...
              channel=await categ.create_text_channel(name=f'paid-{client.name}')
       ...
       @discord.ui.button(label = 'Invites Ticket', style=discord.ButtonStyle.gray, custom_id='Gray:VFX')
       async def VFX_button(self, button = discord.ui.Button, interaction = discord.Interaction):
              ...
              client=interaction.user
              categ= discord.utils.get(interaction.guild.categories, id=cfg.ORDER_TICK)
              channel=await categ.create_text_channel(name=f'invites-{client.name}')
              await channel.edit(overwrites={})
              await channel.set_permissions(interaction.guild.default_role, view_channel=False)
              ...
              embed=discord.Embed(title = 'Invites Ticket', description=f'Ticket Opened by {client.mention}\nHelp will be with you shortly.\nCheck pricing and other information by using `{cfg.PREFIX}procedure`, `{cfg.PREFIX}price`, `{cfg.PREFIX}form`.', color=discord.Color(0x2F3136))
              embed.timestamp = discord.utils.utcnow()
              await channel.send(content=f'{client.mention} | {artists.mention}', embed= embed)
...
class Orders(commands.Cog):
       ...
       async def order_button(self, ctx):
              """Sends Order Button."""
              embed=discord.Embed(title ='Place Order', description=f'Select Your order Type from buttons below to place an order', color=0x2F3136)
              embed.timestamp = discord.utils.utcnow()
