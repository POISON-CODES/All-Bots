...
class TICKET(discord.ui.View):
       ...
       async def ticketopen(self, button = discord.ui.Button, interaction = discord.Interaction):
              ...
              embed.set_footer(text = f'{interaction.guild.name} | {interaction.guild.id}', icon_url = interaction.guild.icon.url if interaction.guild.icon else interaction.user.avatar.url)
              view=CLOSE()
...
class Ticket(commands.Cog):
       ...
       async def TICK(self, ctx):
              embed = discord.Embed(title = 'TICKET', description = f'PRESS THE BUTTON TO OPEN A TICKET', color = discord.Color(0x2F3136))
              embed.timestamp = discord.utils.utcnow()
              embed.set_footer(text = f'{ctx.guild.name} | {ctx.guild.id}', icon_url = ctx.guild.icon.url if ctx.guild.icon else ctx.author.avatar.url)
              view= TICKET()
