...
class BUTTON2(discord.ui.View):
       ...
       async def complete(self, button = discord.ui.Button, interaction = discord.Interaction):
              ...
              if interaction.user.id == artist.id:
                     ...
                     await interaction.response.send_message(f'{artist.mention} artist claimed this order and is currently working on it. This order cannot be claimed by you.', ephemeral = True)
                     return
       ...
       async def reject(self, button = discord.ui.Button, interaction = discord.Interaction):
              ...
              if interaction.user.id == artist.id:
                     ...
                     await interaction.response.send_message(f'You did not claim this Order, hence it cannot be cancelled by you.', ephemeral = True)
                     return
       ...
       async def cancel(self, button = discord.ui.Button, interaction = discord.Interaction):
              ...
              if interaction.user.id == client.id:
                     ...
                     await interaction.response.send_message(f'You are not the client of this particular Order. You cannot cancel it.', ephemeral = True)
                     return
              ...
              embed = discord.Embed(title = 'Order Cancelled', description = f'Order Placed by {client.mention} in **{interaction.guild.name} was cancelled by them.**', color=discord.Color(0xFF1100))
              embed.timestamp = discord.utils.utcnow()
              embed.set_thumbnail(url=interaction.guild.icon)
              embed.set_footer(text = f'{interaction.guild.name} | {interaction.guild.id}')
